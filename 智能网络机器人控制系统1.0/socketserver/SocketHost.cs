...
namespace SocketServer
{
    public class SocketHost
    {
        ...
        {
            String context = "{\"DateTime\":\"1970-01-01 08:00:00\",\"Method\":\"Send\",\"Param\":{\"Message\":[\"" + str + "\"],\"UserName\":\"ROBOT\"}}";
            SocketRequest request = null;
            JavaScriptSerializer jss = new JavaScriptSerializer();
            request = jss.Deserialize(context, typeof(SocketRequest)) as SocketRequest;

            //{"DateTime":"1970-01-01 08:00:00","Method":"Send","Param":{"Message":["拒绝"],"UserName":"Xyz"}}
            SendResponseManager arm = new SendResponseManager();
            arm.Write(socket, this.socketClientSesson.Select(s => s.Key).ToList(),
                request.Param as IDictionary<string, object>);
        }
